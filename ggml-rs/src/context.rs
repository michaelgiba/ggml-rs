...
impl Context {
    ...
    pub fn new_tensor_1d(&self, dtype: DataType, ne0: i32) -> Tensor {
        let raw =
            unsafe { ggml_internal::ggml_new_tensor_1d(self.ptr.as_ptr(), dtype.into(), ne0) };
        self.new_tensor_raw(raw)
    }

    pub fn new_tensor_2d(&self, dtype: DataType, ne0: i32, ne1: i32) -> Tensor {
        let raw =
            unsafe { ggml_internal::ggml_new_tensor_2d(self.ptr.as_ptr(), dtype.into(), ne0, ne1) };
        self.new_tensor_raw(raw)
    }

    pub fn new_tensor_3d(&self, dtype: DataType, ne0: i32, ne1: i32, ne2: i32) -> Tensor {
        let raw = unsafe {
            ggml_internal::ggml_new_tensor_3d(self.ptr.as_ptr(), dtype.into(), ne0, ne1, ne2)
        };
        self.new_tensor_raw(raw)
    }
