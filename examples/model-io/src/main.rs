...
mod tests {
    ...
    fn test_reader_1d() {
        ...
        assert!(
            FourU8Parameter::read_to_tensor(&ctx, &mut reader, Dimension::D1, vec!(None)).is_err()
        );
    }
    ...
    fn test_reader_2d() {
        ...
        for _ in 0..2 {
            let read_result =
                RectU8Param::read_to_tensor(&ctx, &mut reader, Dimension::D2, vec![Some(16), Some(2)]);
            assert!(read_result.is_ok());
            let tensor = read_result.unwrap();
            assert_eq!(tensor.nbytes(), 32);
        }
        assert!(
            RectU8Param::read_to_tensor(&ctx, &mut reader, Dimension::D2, vec!(None, None)).is_err()
        );
    }    
    ...
    fn test_reader_3d() {
        ...
        for _ in 0..8 {
            let read_result =
                CubeU8Param::read_to_tensor(&ctx, &mut reader, Dimension::D3, vec![Some(2), Some(2), Some(2)]);
            assert!(read_result.is_ok());
            let tensor = read_result.unwrap();
            assert_eq!(tensor.nbytes(), 8);
        }
        assert!(
            CubeU8Param::read_to_tensor(&ctx, &mut reader, Dimension::D3, vec!(None, None, None)).is_err()
        );
    }    
